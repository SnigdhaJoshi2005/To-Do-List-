...
export default function Profile() {
  ...
  return (
    <div className="py-6" style={{ animation: "fadeIn 0.4s ease-out" }}>
      <h1 className="font-[var(--font-display)] text-3xl text-primary mb-6">
        Profile
      </h1>
      ...
      <div className="bg-surface border border-border rounded-[var(--radius-xl)] p-7 mb-5 shadow-[var(--shadow-sm)]">
        <div className="flex items-center gap-5 mb-6">
          ...
          <div className="flex-1">
            <h2 className="font-[var(--font-display)] text-[28px] text-primary mb-1">
              {username}
            </h2>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-3">
          {[
            ...
            {
              icon: "🏡",
              value: `${placedCount}/${ownedCount}`,
              label: "Items Placed",
            },
          ].map((s) => (
            <div
              key={s.label}
              className="text-center py-4 bg-muted/80 rounded-[var(--radius-lg)] border border-border/30 transition-all duration-[var(--transition)] hover:shadow-[var(--shadow-sm)] hover:-translate-y-px"
            >
              <span className="block text-2xl mb-1.5">{s.icon}</span>
              <span className="block text-xl font-extrabold text-primary tabular-nums">
                {s.value}
              </span>
      ...
      <div className="bg-surface border border-border rounded-[var(--radius-xl)] p-7 mb-5 shadow-[var(--shadow-sm)]">
        <h3 className="font-[var(--font-display)] text-xl text-primary mb-5">
          Customization
        </h3>
            ...
            <div key={group.key} className="flex flex-col gap-2.5">
              ...
              <div className="flex gap-2.5">
                {group.options.map((o) => (
                  <button
                    key={o.value}
                    className={`w-14 h-14 rounded-[var(--radius-lg)] bg-muted border-2 border-border flex items-center justify-center text-2xl transition-all duration-[var(--transition)] hover:border-accent-alt hover:scale-105 active:scale-95 ${
                      avatar[group.key] === o.value
                        ? "border-accent-alt bg-[rgba(212,165,165,0.12)] shadow-[0_0_0_3px_rgba(212,165,165,0.2)]"
                        : ""
                    }`}
                    onClick={() =>
                      setAvatar((p) => ({ ...p, [group.key]: o.value }))
                    }
                  >
                    {o.icon}
                  </button>
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3">
          {inventory
            ...
            .map((item) => (
              <div
                key={item.id}
                className="flex flex-col items-center gap-1.5 py-4 bg-muted/80 rounded-[var(--radius-lg)] border border-border/30 transition-all duration-[var(--transition)] hover:shadow-[var(--shadow-sm)] hover:-translate-y-px"
              >
